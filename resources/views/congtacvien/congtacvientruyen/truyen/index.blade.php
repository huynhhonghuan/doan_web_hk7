...
@section('content')
    <p class="mt-3">
        <a href="{{ route('congtacvientruyen.truyen.create') }}" class="btn btn-warning"><svg xmlns="http://www.w3.org/2000/svg"
        ...
        <a href="{{ route('congtacvientruyen.truyen.xuat') }}" class="btn btn-success"><svg xmlns="http://www.w3.org/2000/svg"
            ...
            Xuất ra Excel</a>
        <a href="{{ route('congtacvientruyen.truyen.hinh') }}" class="btn btn-danger"><svg xmlns="http://www.w3.org/2000/svg"
            ...
            @if (empty($danhsach) == false)
                @foreach ($danhsach as $key => $truyen)
                    <tr>
                        ...
                        <td>{{ $truyen->getTheLoai->pluck('tentheloai')->first() }}</td>
                        ...
                        <td>
                            <a class="btn btn-primary btn-sm"
                                href="{{ route('congtacvientruyen.truyen.edit', [$truyen]) }}"><i
                                    class="fas fa-edit"></i></a>
                            <form onclick="return confirm('Bạn có muốn xóa không?');" class="d-inline-block"
                                action="{{ route('congtacvientruyen.truyen.destroy', [$truyen]) }}" method="post">
                                @csrf
                                @method('delete')
    ...
    <form action="{{ route('congtacvientruyen.truyen.nhap') }}" method="post" enctype="multipart/form-data">
        @csrf
